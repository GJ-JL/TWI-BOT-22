...
class BotGAT(nn.Module):
    ...
    def forward(self, des, tweet, num_prop, cat_prop, edge_index, edge_type=None):
        ...
        # 3. 关键修改：提取分类前的高维嵌入（linear_relu_output1的输出）
        high_dim_embedding = self.linear_relu_output1(x)  # 高维嵌入：(节点数, hidden_dim)，供可视化

        # 4. 最终分类输出（原有逻辑不变）
        x = self.linear_output2(high_dim_embedding)  # 分类输出：(节点数, 2)

        # 5. 返回两个值：分类输出（用于计算指标）、高维嵌入（用于可视化）
        return x, high_dim_embedding
