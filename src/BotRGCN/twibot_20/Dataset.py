...
class Twibot22(Dataset):
    def __init__(self, root, device='cpu',process=True,save=True):
        self.root = os.path.join(root, '')  # 核心修改：无论输入是否带斜杠，强制补全斜杠
        self.device = device
        if process:
    ...
    def load_labels(self):
        ...
        # print(f"实际查找的标签文件路径：{path}")  # 新增打印语句
    ...
    def Des_embbeding(self):
        ...
        path=self.root+"des_tensor_enh.pt"
        if not os.path.exists(path):
            ...
            des_tensor=torch.load(self.root+"des_tensor_enh.pt").to(self.device)
        print('Finished')
        return des_tensor
    ...
    def tweets_embedding(self):
        ...
        path=self.root+"tweets_tensor_enh.pt"
        if not os.path.exists(path):
            ...
            tweets_tensor=torch.load(self.root+"tweets_tensor_enh.pt").to(self.device)
        print('Finished')
