...
print('loading raw data')   # 改成数据集服务器所在的地址
node=pd.read_json("/home/gaojie/TwiBot-22-master/src/BotRGCN/twibot_20/datasets/Twibot-20/node.json")
edge=pd.read_csv("/home/gaojie/TwiBot-22-master/src/BotRGCN/twibot_20/datasets/Twibot-20/edge.csv")
label=pd.read_csv("/home/gaojie/TwiBot-22-master/src/BotRGCN/twibot_20/datasets/Twibot-20/label.csv")
split=pd.read_csv("/home/gaojie/TwiBot-22-master/src/BotRGCN/twibot_20/datasets/Twibot-20/split.csv")
...
#cat_properties  原来的类别特收集了三个最后确只用了一个
...
#新的cat类别有三个特征了
print('extracting cat_properties')
# 1. default_profile_image
default_profile_image = []
for each in user['profile_image_url']:
    if each is not None:
        # 注意：你原字符串末尾有个空格！建议用 strip() 或修正判断
        if each.strip() == 'http://abs.twimg.com/sticky/default_profile_images/default_profile_normal.png':
...
# 2. protected
protected_list = []
for each in user['protected']:
    # 同样注意：'True ' 有空格！应统一处理
    if str(each).strip() == 'True':
        protected_list.append(1)
    else:
        protected_list.append(0)
...
# 转为 tensor 并拼接
default_profile_image_tensor = torch.tensor(default_profile_image, dtype=torch.float).unsqueeze(1)  # [N, 1]
protected_tensor = torch.tensor(protected_list, dtype=torch.float).unsqueeze(1)                     # [N, 1]
verified_tensor = torch.tensor(verified_list, dtype=torch.float).unsqueeze(1)                      # [N, 1]

# 拼接成 [N, 3]
cat_properties_tensor = torch.cat([default_profile_image_tensor, protected_tensor, verified_tensor], dim=1)

print("cat_properties_tensor shape:", cat_properties_tensor.shape)  # 应该是 torch.Size([229580, 3])
