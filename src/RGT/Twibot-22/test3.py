...
class RGTDetector(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        tweet_features = train_batch.x[:,
                         args.cat_num + args.numeric_num: args.cat_num + args.numeric_num + args.tweet_channel]
        des_features = train_batch.x[:,
                       args.cat_num + args.numeric_num + args.tweet_channel: args.cat_num + args.numeric_num + args.tweet_channel + args.des_channel]
        label = train_batch.y
...
parser.add_argument("--path", type=str, default="/home/gaojie/BotDGT-master/data/Twibot-22/processed_data/",
                    help="dataset path")
...
if __name__ == "__main__":
    ...
    print("loading...")
    train_loader = NeighborLoader(data, num_neighbors=[256] * 4, input_nodes=data.train_idx, batch_size=args.batch_size,
                                  shuffle=True)
    valid_loader = NeighborLoader(data, num_neighbors=[256] * 4, input_nodes=data.valid_idx,
    ...
    trainer = pl.Trainer(
        ...
        devices=[0],  # 使用 1 块 GPU（替代原来的 gpus=1）
        num_nodes=1,
